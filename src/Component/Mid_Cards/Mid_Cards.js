  ...
  const cards = useMemo(
    () => [
      ...
      },
      {
        ...
        description:
          'Building scalable Layer 1 networks and DeFi ecosystems.',
        Icon: "/Images/midsection/tok.svg",
      },
      {
    ...
    <section className="mid-cards-section">
      ...
      <div className="nav-controls">
        <button
          ...
          className={`nav-btn ${atStart ? 'is-white' : 'is-black'}`}
          onClick={() => handleNav('left')}
          disabled={atStart}
        >
          <img src="/Images/midsection/arrowleft.svg" alt="left" />
        </button>
        <button
          type="button"
          aria-label="Scroll right"
          className={`nav-btn ${atEnd ? 'is-white' : 'is-black'}`}
          onClick={() => handleNav('right')}
        >
          <img src="/Images/midsection/arrowright.svg" alt="right" />
        </button>
      </div>
    </section>
  )
